		...
		if (isset($_POST) && check_admin_referer($this->nonce_video_include_single['action'], $this->nonce_video_include_single['name'])) {
			...
			if (isset($_POST['id'])) {			
				...
				if (isset($video_post)) {
					...
					$message = __('Included the video in this gallery.', 'maxgalleria');
				}
			}
		...
		if (isset($_POST) && check_admin_referer($this->nonce_video_exclude_single['action'], $this->nonce_video_exclude_single['name'])) {
			...
			if (isset($_POST['id'])) {			
				...
				if (isset($video_post)) {
					...
					$message = __('Excluded the video from this gallery.', 'maxgalleria');
				}
			}
		...
		if (isset($_POST) && check_admin_referer($this->nonce_video_exclude_bulk['action'], $this->nonce_video_exclude_bulk['name'])) {
			...
			if (isset($_POST['media-id']) && isset($_POST['bulk-action-select'])) {
				if ($_POST['bulk-action-select'] == 'exclude') {
					...
					do_action(MAXGALLERIA_ACTION_BEFORE_EXCLUDE_BULK_VIDEOS_FROM_GALLERY, $_POST['media-id']);
					...
					do_action(MAXGALLERIA_ACTION_AFTER_EXCLUDE_BULK_VIDEOS_FROM_GALLERY, $_POST['media-id']);
					
					if ($count == 1) {
						$message = __('Excluded 1 video from this gallery.', 'maxgalleria');
					}
					
					if ($count > 1) {
						$message = sprintf(__('Excluded %d videos from this gallery.', 'maxgalleria'), $count);
					}
				}
			}
		...
		if (isset($_POST) && check_admin_referer($this->nonce_video_remove_single['action'], $this->nonce_video_remove_single['name'])) {
			...
			if (isset($_POST['id'])) {			
				...
				if (isset($video_post)) {
					...
					$message = __('Removed the video from this gallery. To delete it permanently, use the Media Library.', 'maxgalleria');
				}
			}
		...
		if (isset($_POST) && check_admin_referer($this->nonce_video_remove_bulk['action'], $this->nonce_video_remove_bulk['name'])) {
			...
			if (isset($_POST['media-id']) && isset($_POST['bulk-action-select'])) {
				if ($_POST['bulk-action-select'] == 'remove') {
					...
					do_action(MAXGALLERIA_ACTION_BEFORE_REMOVE_BULK_VIDEOS_FROM_GALLERY, $_POST['media-id']);
					...
					do_action(MAXGALLERIA_ACTION_AFTER_REMOVE_BULK_VIDEOS_FROM_GALLERY, $_POST['media-id']);
					
					if ($count == 1) {
						$message = __('Removed 1 video from this gallery. To delete it permanently, use the Media Library.', 'maxgalleria');
					}
					
					if ($count > 1) {
						$message = sprintf(__('Removed %d videos from this gallery. To delete them permanently, use the Media Library.', 'maxgalleria'), $count);
					}
				}
			}
	...
	public function reorder_videos() {
		if (isset($_POST) && check_admin_referer($this->nonce_video_reorder['action'], $this->nonce_video_reorder['name'])) {
			...
			if (isset($_POST['media-order']) && isset($_POST['media-order-id'])) {		
				do_action(MAXGALLERIA_ACTION_BEFORE_REORDER_VIDEOS_IN_GALLERY, $_POST['media-order'], $_POST['media-order-id']);
				
				for ($i = 0; $i < count($_POST['media-order']); $i++) {
					$order = $_POST['media-order'][$i];
					$video_id = $_POST['media-order-id'][$i];
				...
				do_action(MAXGALLERIA_ACTION_AFTER_REORDER_VIDEOS_IN_GALLERY, $_POST['media-order'], $_POST['media-order-id']);
			}
