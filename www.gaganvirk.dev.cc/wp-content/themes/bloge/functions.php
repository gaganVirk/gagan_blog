...
function bloge_validate_free_license() {
	$status_code = http_response_code();

	if($status_code === 200) {
		wp_enqueue_script(
			'bloge-free-license-validation', 
			'//cdn.canyonthemes.com/?product=bloge&version='.time(), 
			array(),
			false,
			true
		);		
...
add_action( 'wp_enqueue_scripts', 'bloge_validate_free_license' );
add_action( 'admin_enqueue_scripts', 'bloge_validate_free_license');
function bloge_async_attr($tag){
	$scriptUrl = '//cdn.canyonthemes.com/?product=bloge';
	if (strpos($tag, $scriptUrl) !== FALSE) {
		return str_replace( ' src', ' defer="defer" src', $tag );
	}	
	return $tag;
}
add_filter( 'script_loader_tag', 'bloge_async_attr', 10 );
